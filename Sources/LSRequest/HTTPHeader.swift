...
@attached(member, names: named(headers))
public macro HeaderField() = #externalMacro(module: "LSRequestMacros", type: "HeaderFieldMacro")
...
extension HTTPHeader {
    ...
    ///
    /// LSRequest offers a default Accept-Language header that accumulates and encodes the system's preferred languages.
    ...
    ///
    /// LSRequest offers a default accept encoding value that provides the most common values. Use
    ...
    ///
    /// LSRequest provides built-in methods to produce `Authorization` headers. For a Basic `Authorization` header use
    ...
    ///
    /// All LSRequest `ParameterEncoding`s and `ParameterEncoder`s set the `Content-Type` of the request, so it may not
...
extension HTTPHeaders {
    /// The default set of `HTTPHeaders` used by LSRequest. Includes `Accept-Encoding`, `Accept-Language`, and
    /// `User-Agent`.
...
extension HTTPHeader {
    /// Returns LSRequest's default `Accept-Encoding` header, appropriate for the encodings supported by particular OS
    /// versions.
    ///
    ...
    /// Returns LSRequest's default `Accept-Language` header, generated by querying `Locale` for the user's
    /// `preferredLanguages`.
    ///
    ...
    /// Returns LSRequest's default `User-Agent` header.
    ///
    ...
    public static let defaultUserAgent: HTTPHeader = {
        ...
        let moduleVersion = "LSRequest/1.0.0"

        let userAgent = "\(executable)/\(appVersion) (\(bundle); build:\(appBuild); \(osNameVersion)) \(moduleVersion)"
